...
vector<vector<int>> generateDensityMap(){
    ...
    if (!filesystem::exists(filePath)) throw std::runtime_error("Error: File " + filePath + " does not exist.");
    ...
    try{
        ...
        if (cellSize <= 0)  throw std::runtime_error("Error: Cell size must be positive.");
        ...
        cerr << "PDAL error occurred: " << e.what() << endl;
        return vector<vector<int>>();
        
    } catch (const cv::Exception &e) {

        cerr << "OpenCV error occurred: " << e.what() << endl;
        return vector<vector<int>>(); 
